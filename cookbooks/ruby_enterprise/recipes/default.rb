...
packages = value_for_platform(
    ["centos","redhat","fedora"] => {'default' => ['readline-devel', 'openssl-devel', 'patch', 'gcc', 'gcc-c++']},
...
bash "Install Ruby Enterprise Edition" do
  ...
  ruby-enterprise-#{node[:ruby_enterprise][:version]}/installer \
    --auto=#{node[:ruby_enterprise][:install_path]} --dont-install-useful-gems --no-dev-docs || /bin/true
  EOH
  not_if do
